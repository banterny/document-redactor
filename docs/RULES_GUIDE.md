...
- [ ] **The guard is not enough on its own — also nest an optional separator so it gates its own whitespace run.** `\s*:?\s*` is two adjacent unbounded runs separated by an optional, usually-absent colon, which gives the engine O(k) equivalent ways to split a k-character whitespace run before concluding the label isn't there. Write `\s*(?::\s*)?`. It accepts exactly the same language — a whitespace run, optionally interrupted by one colon — and both runs stay unbounded, so unlike a `{0,n}` bound it costs no cliff. **A version boundary makes this easy to miss:** `entities.ko-phone-context` and `entities.uk-inquest-context` carried the guard and measured **0.00ms on V8 13.6** (node 24) while measuring **93.8ms and 133.7ms on V8 12.4** (node 22 — and Chrome ~124) on the same machine and input. The guard's effectiveness depends on an optimisation that older V8 does not have, so a rule can look fixed on a current runtime and still freeze a browser one version behind. Nesting fixes it on both: 0.10ms and 0.09ms.
- [ ] **When the body CAN start with whitespace, guard on a necessary condition instead.** The guard above needs a body that cannot begin with a space. `legal.uk-legal-context` has body `[^\n;,]{3,80}(?=$|\n|[;,])`, which legitimately can — that is what lets a label at end-of-line bridge to an indented value. Derive a cheap assertion the body *implies* rather than one it forbids: if 81 characters of `[^\n;,]` follow, then for every body length in `[3,80]` the character at that offset is neither end-of-input nor a delimiter, so no match can start here. `(?![^\n;,]{81})` is therefore exact, and an EXACT count `{81}` costs one bounded scan with no backtracking. Measured 78.0ms → 2.9ms on JavaScriptCore at 10,000 whitespace characters (real Safari 26).
- [ ] **Factor a shared connector out of parallel lookbehinds.** `(?<=aX)|(?<=bX)|(?<=cX)` is exactly `(?<=(?:a\|b\|c)X)` when `X` is shared — a lookbehind succeeds iff some suffix matches it, so the union of the three languages is the language of the alternation. It matters when `X` is the expensive part: three alternatives meant walking the same `\s{0,100}(?::\s{0,100})?` three times per position. If the alternatives end in different characters, a nested `(?<=[.oef])` placed where the engine arrives after consuming the run rejects a bad split in one comparison instead of retrying every literal; it is implied by the alternation, so it is exact.
- [ ] **A greedy run before a literal is a different bug, and the guard hoist does nothing for it.** `(?<!X)(?:[A-Za-z0-9][A-Za-z0-9&.\-]*)\s+LITERAL` has no positive lookbehind to short-circuit. The run consumes to end of input, `\s+` fails, and the engine backtracks through every length — retried at every position the negative lookbehind admits, which is quadratic. It bites on **digits and `a-` runs, not whitespace**. Atomic-group emulation `(?=(X))\1` alone is not enough (measured 4.4× faster and still quadratic): it removes the backtracking but not the O(n) rescan at O(n) start positions. Split the start positions instead — an unbounded branch gated on a *hard boundary* (predecessor outside the run's own character class, which makes the maximal runs provably disjoint and the branch linear) plus a bounded branch for genuine mid-token starts. See `entities.ko-corp-suffix`.
...
**Time a ReDoS assertion with `expectWithinBudget`, never `performance.now()`.** Each rule test file keeps its own thin `expectFast` wrapper — they differ in what they invoke — but the timing itself comes from `tests/helpers/redos-budget.ts`, which bills **thread CPU** via `process.threadCpuUsage()`. These assertions run in-process under `pool: "threads"`, so a wall-clock budget measures scheduler pressure as much as it measures the rule: running the suite under 32 CPU hogs on 8 cores failed seven of them across five files, every one on a rule that passes the cross-engine deep gate comfortably. The same instrument change took that to zero. Do not reach for `process.cpuUsage()` instead — it is process-*wide*, so under the threads pool an idle test thread is billed every sibling's burn on every core, which is worse than wall-clock. `tests/helpers/redos-budget.test.ts` fails if the thread-scoped clock ever silently falls back. Budgets should still carry headroom (CPU time inflates a little under cache pressure and frequency scaling) — but headroom over the *measured cost*, not over the scheduler.
